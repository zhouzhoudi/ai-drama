...
This module splits the Director Agent decision step into focused sub-agents while
keeping the existing backend contract unchanged. It returns the same decision
shape as the legacy AgentRouter._decide():

{
    "action": "CREATE_SCRIPT | REWRITE_SCRIPT | ...",
    "reply": "中文回复",
    "parameters": {...},
    "ui_patch": {"active_tab": "canvas|script|characters|storyboard"}
}

The actual execution still lives in the existing AgentRouter. This layer only
chooses which specialist should handle the user's next instruction.
"""
...
class MultiAgentOrchestrator:
    ...
    async def decide(
        ...
        selected = votes[0] if votes else ChatAgent().vote(ctx)  # type: ignore[assignment]
